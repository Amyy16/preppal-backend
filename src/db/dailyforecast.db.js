...
const DailyForecastRepository = {
  ...
  // In DailyForecastRepository.js
  async getForecastsByDateRange(startDate, endDate, businessId) {
    try {
      const forecasts = await DailyForecast.findAll({
        where: {
          forecastDate: {
  ...
  async getForecastByItemAndDate(startDate, endDate, businessId, itemId) {
    try {
      ...
      throw new Error(
        "Error fetching forecast by item and date: " + error.message,
      );
    }
  },
